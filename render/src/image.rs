...
impl ImageData {
    pub fn rgba_data(&self) -> &[u8] {
        ...
        unsafe { std::slice::from_raw_parts(ptr.cast(), 4 * len) }
    }
}
...
pub fn load_image(
    ...
    let bits_per_component = image.bits_per_component.ok_or_else(|| PdfError::Other {
        msg: format!("no bits per component"),
    })?;
    ...
    if raw_data.len() % pixel_count != 0 {
        warn!(
            "invalid data length {} bytes for {} pixels",
            raw_data.len(),
            pixel_count
        );
    ...
    let alpha = match mask {
        Some(ref mask) => {
            ...
            let bits_per_component = mask.bits_per_component.ok_or_else(|| PdfError::Other {
                msg: format!("no bits per component"),
            })?;
            ...
            let mut alpha: Data = match bits_per_component {
                ...
                2 => data
                    .iter()
                    .flat_map(|&b| (0..4).map(move |i| ex(b >> 2 * i, 2)))
                    .collect::<Vec<u8>>()
                    .into(),
                4 => data
                    .iter()
                    .flat_map(|&b| (0..2).map(move |i| ex(b >> 4 * i, 4)))
                    .collect::<Vec<u8>>()
                    .into(),
                8 => data,
                12 => data
                    .chunks_exact(3)
                    .flat_map(|c| [c[0], c[1] << 4 | c[2] >> 4])
                    .collect::<Vec<u8>>()
                    .into(),
                16 => data
                    .chunks_exact(2)
                ...
                n => {
                    return Err(PdfError::Other {
                        msg: format!("invalid bits per component {}", n),
                    })
                }
            };
            if mask.width != image.width || mask.height != image.height {
                alpha = resize_alpha(&*alpha, mask.width, mask.height, image.width, image.height)
                    .unwrap()
                    .into();
            }
            alpha
    ...
    let cs = image
        .color_space
        .as_ref()
        .and_then(|cs| resolve_cs(cs, &resources));
    ...
    let data = match data_ratio {
            ...
            match cs {
                ...
                Some(ColorSpace::Indexed(ref base, ref lookup)) => {
                    match resolve_cs(&**base, resources) {
                        Some(ColorSpace::DeviceRGB) => pixel_data
                            .iter()
                            .zip(alpha)
                            .map(|(&b, a)| {
        ...
        24 => {
            if !matches!(cs, Some(ColorSpace::DeviceRGB)) {
                info!(
                    "image has data/pixel ratio of 3, but colorspace is {:?}",
                    cs
                );
            }
            raw_data
        ...
        32 => {
            if !matches!(cs, Some(ColorSpace::DeviceCMYK)) {
                info!(
                    "image has data/pixel ratio of 4, but colorspace is {:?}",
                    cs
                );
            }
